...
drop policy if exists p_rooms_read on public.rooms;
...
drop policy if exists p_slots_read on public.availability_slots;
create policy p_slots_read on public.availability_slots for select to authenticated using (true);

drop policy if exists p_slots_write on public.availability_slots;
...
drop policy if exists p_res_read on public.reservations;
...
drop policy if exists p_res_insert on public.reservations;
create policy p_res_insert on public.reservations for insert to authenticated
  with check (user_id = auth.uid());

drop policy if exists p_res_update on public.reservations;
